        ...
        .then(({ textInput, textColor, shape, shapeColor }) => {
          ...
          if (shape === "Circle") {
            ...
            fs.writeFileSync(
              ...
              `<svg version="1.1"
            width="300" height="200"
            xmlns="http://www.w3.org/2000/svg"><${shape} points="150, 18 244, 182 56, 182" fill="${shapeColor}" /><text x="50%" y="50%" text-anchor="middle" fill="${textColor}">${textInput}</text></svg>`
            );
          } else {
            fs.writeFileSync(
              "logo.svg",
              `<svg version="1.1"
            width="300" height="200"
            xmlns="http://www.w3.org/2000/svg"><${shape} x="90" y="40" width="120" height="120" fill="${shapeColor}" /><text x="50%" y="50%" text-anchor="middle" fill="${textColor}">${textInput}</text></svg>`
            );
          }
        })
        .then(() => {
